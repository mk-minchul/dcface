    ...
    def forward(self, x, emb, cross_attn=None, stylemod=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                x = layer(x, cross_attn=cross_attn)
            else:
                x = layer(x)
        return x
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, cross_attn=None):
        if self.use_checkpoint:
            return checkpoint(self._forward, (x, cross_attn), self.parameters(), True)
        else:
            return self._forward(x, cross_attn)
...
class UNetModel(nn.Module):
                ...
                if ds in attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ...
                            encoder_channels=cross_attn_encoder_channels
                        )
                    )
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ResBlock(
                            ...
                            use_checkpoint=use_checkpoint,
                            use_scale_shift_norm=use_scale_shift_norm,
                            down=True if level != skip_downsample_level else False,
                        )
                        if resblock_updown
                        else Downsample(
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ResBlock(
                            ...
                            use_checkpoint=use_checkpoint,
                            use_scale_shift_norm=use_scale_shift_norm,
                            up=True if level != (skip_downsample_level+1) else False,
                        )
                        if resblock_updown
    ...
    def forward(self, x, timesteps, encoder_hidden_states=None, y=None, ref_img=None):
        """
